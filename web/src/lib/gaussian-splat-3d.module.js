...
class Viewer {
    ...
    return function () {
      ...
      if (vertexCount > 0) {
        ...
        this.cameraFocalLength =
          renderDimensions.y /
          2.0 /
          Math.tan((THREE_CAMERA_FOV / 2.0) * THREE.MathUtils.DEG2RAD);
  ...
  createDebugMeshes(renderOrder) {
    ...
    let sphereMesh = new THREE.Mesh(
      sphereGeometry,
      this.buildDebugMaterial(0xff0000)
    );
    ...
    sphereMesh = new THREE.Mesh(
      sphereGeometry,
      this.buildDebugMaterial(0x00ff00)
    );
    sphereMesh.renderOrder = renderOrder;
    debugMeshRoot.add(sphereMesh);
    ...
    sphereMesh = new THREE.Mesh(
      sphereGeometry,
      this.buildDebugMaterial(0xffaa00)
    );
    sphereMesh.renderOrder = renderOrder;
    debugMeshRoot.add(sphereMesh);
  ...
  createSecondaryDebugMeshes(renderOrder) {
    ...
    let boxMesh = new THREE.Mesh(
      boxGeometry,
      this.buildDebugMaterial(boxColor)
    );
  ...
  gatherSceneNodes = (function () {
    ...
    return function (gatherAllNodes) {
      ...
      const fovXOver2 = Math.atan(
        renderDimensions.x / (2.0 * this.cameraFocalLength)
      );
      const fovYOver2 = Math.atan(
        renderDimensions.y / (2.0 * this.cameraFocalLength)
      );
  ...
  start() {
    if (this.selfDrivenMode) {
      const requestId = requestAnimationFrame(this.selfDrivenUpdateFunc);
      this.requestId = requestId;
      console.log("Starting viewer.", requestId);
      this.running = true;
    } else {
  ...
  stop() {
    console.log("Stopping viewer.", this.requestId);
    this.renderer.dispose();
    if (this.selfDrivenMode) {
      cancelAnimationFrame(this.requestId);
      this.running = false;
    } else {
      throw new Error("Cannot stop viewer unless it is in self driven mode.");
    }
  }
  ...
  render() {
    ...
    if (this.scene.children.length > 0) {
      ...
      this.renderer.render(this.scene, this.camera);
      this.renderTargetCopyMaterial.uniforms.sourceColorTexture.value =
        this.splatRenderTarget.texture;
      this.renderTargetCopyMaterial.uniforms.sourceDepthTexture.value =
        this.splatRenderTarget.depthTexture;
      this.renderer.render(
        this.renderTargetCopyQuad,
        this.renderTargetCopyCamera
      );
    } else {
      this.renderer.clear(true, true, true);
      this.renderer.render(this.splatMesh, this.camera);
